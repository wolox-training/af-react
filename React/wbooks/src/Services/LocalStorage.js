...
const defineProperty = (prop, defaultKey = '', tag = '') => {
  ...
  module.exports[`set${capitalizedKey}`] = (val, key = defaultKey) =>
    setValue(`@@Backoffice:${prop}${tag}${key}`, val);
  module.exports[`get${capitalizedKey}`] = (key = defaultKey) => getValue(`@@Backoffice:${prop}${tag}${key}`);
  module.exports[`remove${capitalizedKey}`] = (key = defaultKey) =>
    removeValue(`@@Backoffice:${prop}${tag}${key}`);
};

module.exports.getValue = key => getValue(key);
module.exports.removeValue = key => removeValue(key);
module.exports.setValue = (key, value) => setValue(key, value);
